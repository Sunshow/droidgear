...
部分平台的同一个分组同时支持多种 API 协议，无法仅凭 `platform` 推断，例如
sub2api 的 `deepseek` 平台（同时支持 OpenAI / Anthropic / 通用兼容，默认
OpenAI）。这类平台在 `src/lib/sub2api-platform.ts` 中声明：

```typescript
const MULTI_PROTOCOL_PLATFORMS = new Set(['deepseek'])

export const isMultiProtocolPlatform = (platform?: string | null): boolean =>
  !!platform && MULTI_PROTOCOL_PLATFORMS.has(platform.toLowerCase())

/** 可选协议（第一项为默认值）：OpenAI / Anthropic / 通用兼容 */
...
1. 未显式选择时，`Sub2ApiInferrer` 与 `inferProviderFromPlatformAndModel`
   都返回 `openai`；
2. Base URL 与协议相关：OpenAI（Responses）与 Anthropic 使用裸 Base URL，
   通用兼容模式走 OpenAI 兼容端点，必须追加 `/v1` 后缀（见
   `getBaseUrlForSub2Api` / `Sub2ApiInferrer.getBaseUrl`），协议差异由
   `CustomModel.provider` 表达；
3. `ChannelModelPicker` / `ChannelDetail` 在多协议平台下展示协议下拉框，用户
   选择通过 `ChannelProviderContext.provider` 传给调用方；调用方可用
   `providerToModelProtocol` / `providerToClientApiType` 映射到各自的 API 类型，
   并在通用兼容模式下用 `ensureOpenAICompatibleV1` 为 OpenAI 兼容端点补齐
   `/v1` 后缀（OpenClaw 自身已有等价处理）；
4. 模型列表接口：sub2api 的 `/v1/models` 与 DeepSeek 官方的 `/models` 都尝试
   （见 `droidgear-core` 的 `fetch_models_by_api_key`）。
